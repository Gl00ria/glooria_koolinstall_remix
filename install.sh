...
whiptail --title "Gl00ria's DotKool-remix (2025) Install Script" \
  --msgbox "Welcome to Gl00ria's DotKool-remix (2025) Install Script!!!\n\n\
...
options_command+=(
  ...
  "power_management" "Configure Power Management tools eg..(powertop)?" "OFF"
  "backup_utils" "Configure Back up Utils eg..(timeshift)?" "OFF"
  "cpu_intel" "Configure Intel CPU?" "OFF"
  "package_management" "Configure Package Management tools eg..(reflector)?" "OFF"
  "net_tools" "Configure Networking tools eg..(openvpn)?" "OFF"
  "fs_tools" "Configure File System tools eg..(nfs-utils)?" "OFF"
  "txt_editors" "Configure Txt Editors eg..(vim/nvim)?" "OFF"
  "doc_reader" "Configure Dcomuents Reader eg..(libreoffice/zathura)?" "OFF"
  ...
  "screenshot" "Download Screenshot Apps eg..(Grim/Swappy)?" "OFF"
  "clipboard" "Download Clipboard Tools eg..(cliphist)?" "OFF"
  "misc" "Download Misc Tools eg..(tldr/rsync)?" "OFF"
  "tmux" "Download & Configure Tmux?" "OFF"
  "lang_java" "Setup Java Programming Language?" "OFF"
  "lang_python" "Setup Python Programming Language?" "OFF"
  "thunar" "Install Thunar File Manager?" "OFF"
  "ags" "Install AGS v1 for Desktop-Like Overview" "OFF"
  "xdph" "Install XDG-DESKTOP-PORTAL-HYPRLAND (for screen share)?" "OFF"
  "zsh" "Install zsh shell with Oh-My-Zsh?" "OFF"
  "c_bash" "Configure Bash shell?" "OFF"
  "pokemon" "Add Pokemon color scripts to your terminal?" "OFF"
  "rog" "Are you installing on Asus ROG laptops?" "OFF"
  "dots" "Download & install Gl00ria's DotKool-remix Hyprland dotfiles?" "OFF"
)
...
while true; do
  ...
  if [[ "$dots_selected" == "OFF" ]]; then
    ...
    if ! whiptail --title "Gl00ria's DotKool-remix for Hyprland " --yesno \
      "You have not selected to install the pre-configured Gl00ria's DotKool-remix for Hyprland.\n\nKindly NOTE that if you proceed without Dots, Hyprland will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without Gl00ria's DotKool-remix for Hyprland or return to choices/options?" \
  ...
  echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}Gl00ria's DotKool-remix for 🇵🇭 Hyprland Installation...${RESET}" | tee -a "$LOG"
  break
done
...
echo "${INFO} Installing ${SKY_BLUE}Gl00ria's DotKool-remix for Hyprland additional packages...${RESET}" | tee -a "$LOG"
sleep 1
...
for option in "${options[@]}"; do
  ...
  zsh)
    ...
    ;;
  c_bash)
    echo "${INFO} Configuring ${SKY_BLUE}Zsh...${RESET}" | tee -a "$LOG"
    execute_script "bash.sh"
    ;;
  pokemon)
  ...
  rog)
    ...
    ;;
  power_management)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Power Management tools...${RESET}" | tee -a "$LOG"
    execute_script "power_management.sh"
    ;;
  backup_utils)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Back-up Management tools...${RESET}" | tee -a "$LOG"
    execute_script "backup_utils.sh"
    ;;
  cpu_intel)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}CPU INTEL Packages...${RESET}" | tee -a "$LOG"
    execute_script "cpu_intel.sh"
    ;;
  package_management)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Package Management Tools...${RESET}" | tee -a "$LOG"
    execute_script "pkg_management.sh"
    ;;
  net_tools)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Networking Tools...${RESET}" | tee -a "$LOG"
    execute_script "networking.sh"
    ;;
  fs_tools)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}File system Tools...${RESET}" | tee -a "$LOG"
    execute_script "file_sys.sh"
    ;;
  comp_tools)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Compress Tools...${RESET}" | tee -a "$LOG"
    execute_script "compress_util.sh"
    ;;
  git_tools)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Git Tools...${RESET}" | tee -a "$LOG"
    execute_script "git.sh"
    ;;
  txt_editors)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Txt Editors...${RESET}" | tee -a "$LOG"
    execute_script "txt_editors.sh"
    ;;
  doc_reader)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Documents Readers Apps...${RESET}" | tee -a "$LOG"
    execute_script "doc_reader.sh"
    ;;
  print_tools)
  ...
  browser)
    ...
    ;;
  media)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Media Tools...${RESET}" | tee -a "$LOG"
    execute_script "media.sh"
    ;;
  hypervisor)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Hypervisor App...${RESET}" | tee -a "$LOG"
    execute_script "hypervisor.sh"
    ;;
  screenshot)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Screenshot Apps...${RESET}" | tee -a "$LOG"
    execute_script "screenshot.sh"
    ;;
  clipboard)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Clipboard Apps...${RESET}" | tee -a "$LOG"
    execute_script "clipboard.sh"
    ;;
  lang_java)
    echo "${INFO} Installing ${SKY_BLUE}Java Language...${RESET}" | tee -a "$LOG"
    execute_script "lang_java.sh"
    ;;
  lang_python)
    echo "${INFO} Installing ${SKY_BLUE}Python Language...${RESET}" | tee -a "$LOG"
    execute_script "lang_python.sh"
    ;;
  tmux)
    echo "${INFO} Downloading & Configuring ${SKY_BLUE}Tmux...${RESET}" | tee -a "$LOG"
    execute_script "tmux.sh"
    ;;
  misc)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Misc App/s...${RESET}" | tee -a "$LOG"
    execute_script "misc.sh"
    ;;
  dots)
    echo "${INFO} Installing pre-configured ${SKY_BLUE}Gl00ria's DotKool-remix for Hyprland...${RESET}" | tee -a "$LOG"
...
if [[ -d ~/.config/fastfetch/ ]]; then
  printf "${NOTE}Backing-up your${SKY_BLUE}fastfetch${RESET} Config...\n"
  mv ~/.config/fastfetch/ ~/.config/fastfetch_$(date +'%H:%M:%S_%d-%m-%Y')
  printf "${NOTE}Setting-up my${SKY_BLUE}fastfetch${RESET} Config...\n"
...
if pacman -Q hyprland &>/dev/null || pacman -Q hyprland-git &>/dev/null; then
  ...
  printf "${SKY_BLUE}Thank you${RESET} 🫰 for using 🇵🇭 ${MAGENTA}Gl00ria's DotKool-remix for Hyprland${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
  printf "\n%.0s" {1..2}
