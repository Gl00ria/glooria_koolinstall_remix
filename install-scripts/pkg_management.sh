...
LOG="Install-Logs/install-$(date +%d-%H%M%S)_pkg_management.log"

# Package Management
printf "${NOTE} Downloading & Installing ${SKY_BLUE}Package Management${RESET} Packages...\n"
for PMAN in "${package_management[@]}"; do
  install_package "$PMAN" "$LOG"
done
